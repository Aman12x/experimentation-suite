...
# Header
st.markdown(
    '<p class="main-header">🔬 Experimentation & Causal Analysis Suite</p>',
    unsafe_allow_html=True,
)
st.markdown(
    '<p class="subtitle">Professional-grade statistical testing and causal inference platform</p>',
    unsafe_allow_html=True,
)
...
with st.sidebar:
    ...
    if uploaded_file:
        try:
            ...
            st.write(
                f"**Numeric columns:** {len(st.session_state.data_handler.numeric_columns)}"
            )
            st.write(
                f"**Categorical columns:** {len(st.session_state.data_handler.categorical_columns)}"
            )
...
if uploaded_file:
    ...
    with tab1:
        ...
        st.dataframe(
            st.session_state.data_handler.data.describe(), use_container_width=True
        )
            ...
            if not is_valid:
                ...
                with st.spinner("Running analysis..."):
                    ...
                    if health_results["overall_health"]:
                        ...
                        st.warning(
                            "⚠️ Some health checks failed. Review warnings below."
                        )
                    ...
                    for check in health_results["checks_passed"]:
                        st.markdown(
                            f'<div class="success-box">{check}</div>',
                            unsafe_allow_html=True,
                        )
                    ...
                    if test_type == "T-Test":
                        ...
                        st.info(
                            "ℹ️ Converting continuous metric to binary (above median = success)"
                        )
                    ...
                    if test_type != "Bayesian":
                        ...
                        with col1:
                            st.metric(
                                "Control Mean", f"{results.get('control_mean', 0):.4f}"
                            )
                        ...
                        interpretation = (
                            st.session_state.interpreter.interpret_ab_test_results(
                                results
                            )
                        )
                        ...
                        with col2:
                            st.metric(
                                "Expected Lift", f"{results['expected_lift']:+.2f}%"
                            )
                        ...
                        bayesian_interp = (
                            st.session_state.interpreter.interpret_bayesian_results(
                                results
                            )
                        )
                        ...
                        fig_posterior = (
                            st.session_state.visualizer.plot_bayesian_posteriors(
                    ...
                    st.session_state.ab_test_interpretation = (
                        interpretation if test_type != "Bayesian" else bayesian_interp
                    )
        ...
        with st.expander("Calculate Required Sample Size"):
            ...
            with col1:
                baseline_mean = st.number_input(
                    "Baseline Mean", value=100.0, help="Expected mean of control group"
                )

            with col2:
                baseline_std = st.number_input(
                    "Baseline Std Dev", value=20.0, help="Expected standard deviation"
                )
            ...
            if st.button("Calculate Sample Size"):
                ...
                if power_results:
                    st.success(
                        f"✅ Total sample size needed: **{power_results['total_sample_size']:,}**"
                    )

                    power_interp = (
                        st.session_state.interpreter.interpret_power_analysis(
                            power_results
                        )
                    )
    ...
    with tab3:
        ...
        if method == "Propensity Score Matching (PSM)":
            ...
            st.info(
                "PSM creates comparable groups from observational data by matching units with similar characteristics."
            )
            ...
            if st.button("🚀 Run PSM Analysis", type="primary"):
                if not covariate_cols:
                    ...
                    with st.spinner("Running Propensity Score Matching..."):
                        psm_results = (
                            st.session_state.causal_lab.propensity_score_matching(
                        ...
                        if "error" in psm_results:
                            ...
                            with col3:
                                st.metric(
                                    "Match Rate", f"{psm_results['match_rate']:.1f}%"
                                )

                            with col4:
                                sig = (
                                    "✅ Significant"
                                    if psm_results["p_value"] < 0.05
                                    else "❌ Not Significant"
                                )
                            ...
                            causal_interp = (
                                st.session_state.interpreter.interpret_causal_effect(
                                    psm_results, "PSM"
                                )
                            )
                            ...
                            with viz_col1:
                                fig_ps = (
                                    st.session_state.visualizer.plot_propensity_scores(
                            ...
                            st.dataframe(
                                psm_results["balance_stats"], use_container_width=True
                            )
                ...
                with col2:
                    ...
                    if group_col_did and time_col:
                        treatment_group = st.selectbox(
                            ...
                            options=st.session_state.data_handler.data[
                                group_col_did
                            ].unique(),
                        )
            ...
            st.info(
                "IV estimation addresses endogeneity by using an instrument that affects treatment but not the outcome directly."
            )
            ...
            if st.button("🚀 Run IV Analysis", type="primary"):
                with st.spinner("Running Instrumental Variables estimation..."):
                    ...
                    if iv_results:
                        ...
                        with col1:
                            st.metric(
                                "IV Estimate", f"{iv_results['iv_estimate']:+.4f}"
                            )
                        ...
                        with col3:
                            st.metric(
                                "First Stage F-stat",
                                f"{iv_results['first_stage_f_stat']:.2f}",
                            )
    ...
    with tab4:
        ...
        st.write(
            "Export your analysis results in various formats for sharing and archiving."
        )
        ...
        if not (has_ab_results or has_psm_results or has_did_results):
            ...
            selected_export = st.selectbox(
                "Select Results to Export", options=export_options
            )

            export_format = st.radio(
                "Export Format", options=["Excel", "Markdown", "HTML"], horizontal=True
            )
                ...
                with st.spinner("Generating export..."):
                    ...
                    if export_format == "Excel":
                        excel_file = st.session_state.report_gen.create_excel_report(
                            results, st.session_state.data_handler.data, test_type
                        )
                        ...
                        md_report = st.session_state.report_gen.create_markdown_report(
                            results, interpretation, test_type
                        )
                        ...
                        with st.expander("Preview HTML"):
                            st.components.v1.html(
                                html_report, height=600, scrolling=True
                            )
